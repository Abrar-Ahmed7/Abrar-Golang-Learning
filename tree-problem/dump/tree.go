...
func main() {
	...
	// 	log.Printf("tree %s: %v\n", dirPath, err)
	// }
	res, err := jsonTree(dirPath, "  ", "", &r, c, 0)
	if err != nil {
		log.Printf("tree %s: %v\n", dirPath, err)
	}
...
func appendReport(res string, r report, c configs) string {
	if c.json {
		...
		// {"type":"report","directories":6,"files":10}
...
func jsonTree(root, indent, res string, r *report, c configs, depth int) (string, error) {
	...
	res += indent + applyConfigsForJson1(fi, root, c)
	r.dirCount++
	if !fi.IsDir() {
		r.fileCount++
		res += "\n"
...
func applyConfigsForJson1(file os.FileInfo, path string, c configs) string {
	if c.relPath && c.perm {
		return "[" + file.Mode().Perm().String() + "]" + path + "/" + file.Name()
	} else if !c.relPath && c.perm {
		return "[" + file.Mode().Perm().String() + "]" + file.Name()
		...
		if file.IsDir() {
			return "{\"type\":\"directory\",\"name\":\"" + file.Name() + "\""
		}
